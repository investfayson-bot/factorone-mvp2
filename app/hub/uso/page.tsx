              ...
              <thead>
                <tr style={{ background: 'var(--gray-50,#f8f9fa)' }}>
                  {['Agente', 'Chamadas', 'Prompt tokens', 'Completion tokens', 'Total tokens', 'Custo (USD)'].map((h) => (
                    <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'var(--gray-400)', textTransform: 'uppercase', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", borderBottom: '1px solid var(--gray-100)', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {stats.map((s) => {
                  ...
                  return (
                    <tr key={s.agente_id} style={{ borderTop: '1px solid var(--gray-100)' }}>
                      ...
                      <td style={{ padding: '10px 12px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: 'var(--navy)' }}>{s.chamadas.toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: 'var(--gray-500)' }}>{s.prompt_tokens.toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: 'var(--gray-500)' }}>{s.completion_tokens.toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", fontWeight: 600, color: 'var(--navy)' }}>{s.total_tokens.toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '10px 12px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: s.custo_usd > 0.1 ? 'var(--gold)' : 'var(--navy)' }}>${s.custo_usd.toFixed(4)}</td>
                    </tr>
                  )
                })}
              </tbody>
              ...
              {rows.slice(0, 50).map((r, i) => {
                ...
                return (
                  <tr key={i} style={{ borderTop: '1px solid var(--gray-100)' }}>
                    <td style={{ padding: '8px 12px', color: 'var(--gray-500)', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>{new Date(r.created_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</td>
                    ...
                    <td style={{ padding: '8px 12px', color: 'var(--gray-400)', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>{r.modelo}</td>
                    <td style={{ padding: '8px 12px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: 'var(--navy)' }}>{Number(r.total_tokens).toLocaleString('pt-BR')}</td>
                    <td style={{ padding: '8px 12px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", color: 'var(--navy)' }}>${Number(r.custo_usd).toFixed(6)}</td>
                  </tr>
                )
              })}
            </tbody>
