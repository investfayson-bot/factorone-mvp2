    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ background: '#0d1b2a', padding: '18px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 20, fontWeight: 800, color: '#fff' }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 400, color: '#94a3b8', marginLeft: 12 }}>{data.empresa_nome}</span>
        </div>
        <div style={{ fontSize: 14, color: '#94a3b8' }}>Portal do Cliente</div>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 20px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 14, padding: '20px 24px', marginBottom: 20, boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            <div>
              ...
              {data.cliente.cnpj_cpf && <div style={{ fontSize: 14, color: '#94a3b8', marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>{data.cliente.cnpj_cpf}</div>}
              {data.cliente.segmento && <div style={{ fontSize: 14, color: '#64748b', marginTop: 4 }}>{data.cliente.segmento}</div>}
            </div>
            {data.cliente.valor_contrato && (
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 13, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.06em' }}>Contrato</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
          {[
            ...
            <div key={k.l} style={{ background: '#fff', borderRadius: 12, padding: '16px 18px', boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
              <div style={{ fontSize: 13, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>{k.l}</div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
          {([['faturas', 'Faturas'], ['entregas', 'Entregas'], ['contratos', 'Contratos']] as [typeof tabAtiva, string][]).map(([k, l]) => (
            <button key={k} onClick={() => setTabAtiva(k)} style={{
              padding: '8px 16px', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: 'pointer', border: '1px solid',
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15 }}>
                <thead><tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                  {['Descrição', 'Vencimento', 'Valor', 'Status'].map(h => (
                    <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 13, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.06em' }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid #f8fafc' }}>
                        ...
                        <td style={{ padding: '12px 16px' }}>
                          <span style={{ fontSize: 13, padding: '3px 8px', borderRadius: 12, background: s.bg, color: s.color, fontWeight: 600 }}>{s.label}</span>
                        </td>
                      </tr>
                    )
                  })}
                  ...
                  {data.entregas.map((e, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid #f8fafc' }}>
                        ...
                        <td style={{ padding: '12px 16px', color: '#64748b', fontSize: 14 }}>
                          {e.data_entrega ? new Date(e.data_entrega).toLocaleDateString('pt-BR') : '—'}
                        </td>
                        <td style={{ padding: '12px 16px' }}>
                          <span style={{ fontSize: 13, padding: '3px 8px', borderRadius: 12, background: '#f1f5f9', color: s.color, fontWeight: 600 }}>{s.label}</span>
                        </td>
                      </tr>
                    )
                  })}
                ...
                <tbody>
                  {data.oportunidades.map((o, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f8fafc' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ fontSize: 13, padding: '3px 8px', borderRadius: 12, background: '#f1f5f9', color: '#64748b', fontWeight: 600 }}>{o.etapa}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div style={{ textAlign: 'center', marginTop: 32, fontSize: 14, color: '#94a3b8' }}>
