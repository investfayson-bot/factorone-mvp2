...
function FontCard({ name, stack, rec, nota }: { name: string; stack: string; rec?: boolean; nota: string }) {
  return (
    <div style={{ fontFamily: stack, background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 16, overflow: 'hidden', boxShadow: '0 1px 3px rgba(16,24,40,.04)' }}>
      {/* topo */}
      <div style={{ background: '#1C2B2A', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {rec && <span style={{ fontSize: 10, fontWeight: 700, color: '#0F6E56', background: '#EAF5F3', padding: '3px 10px', borderRadius: 20 }}>RECOMENDADA</span>}
      </div>

      <div style={{ padding: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: '#7A8F8E', letterSpacing: '.06em', textTransform: 'uppercase', marginBottom: 12 }}>Visão geral · Julho 2026</div>

        {/* KPIs */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 18 }}>
          {KPIS.map(k => (
            <div key={k.lbl} style={{ border: '0.5px solid #E2E8E7', borderRadius: 12, padding: '12px 14px' }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: '#7A8F8E', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 8 }}>{k.lbl}</div>
              <div style={{ fontSize: 22, fontWeight: 700, color: '#1C2B2A', letterSpacing: '-0.02em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{k.val}</div>
              <div style={{ fontSize: 11, fontWeight: 500, color: k.up ? '#16A085' : '#E74C3C', marginTop: 6 }}>{k.sub}</div>
            </div>
          ))}
        </div>

        {/* Tabela DRE */}
        <div style={{ border: '0.5px solid #E2E8E7', borderRadius: 12, overflow: 'hidden', marginBottom: 16 }}>
          <div style={{ padding: '10px 14px', background: '#F8FAFA', borderBottom: '0.5px solid #E2E8E7', fontSize: 12, fontWeight: 700, color: '#1C2B2A' }}>DRE — resumo</div>
          {DRE.map((row, i) => (
            <div key={row[0]} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 14px', borderBottom: i < DRE.length - 1 ? '0.5px solid #F0F4F3' : 'none', fontSize: 13 }}>
              <span style={{ color: '#374151' }}>{row[0]}</span>
              <span style={{ fontWeight: 700, color: '#1C2B2A', fontVariantNumeric: 'tabular-nums' }}>{row[1]}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'baseline' }}>
          <span style={{ fontSize: 15, fontWeight: 400, color: '#1C2B2A' }}>Regular 400</span>
          <span style={{ fontSize: 15, fontWeight: 500, color: '#1C2B2A' }}>Medium 500</span>
          <span style={{ fontSize: 15, fontWeight: 600, color: '#1C2B2A' }}>Semibold 600</span>
          <span style={{ fontSize: 15, fontWeight: 700, color: '#1C2B2A' }}>Bold 700</span>
          <span style={{ fontSize: 15, fontWeight: 800, color: '#1C2B2A' }}>Extra 800</span>
        </div>
        <div style={{ fontSize: 20, fontWeight: 700, color: '#1C2B2A', marginTop: 12, fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.01em' }}>
...
export default function FontsComparePage() {
  return (
    <div style={{ minHeight: '100vh', background: '#F4F6F5', padding: '32px 16px', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: '#1C2B2A', letterSpacing: '-0.03em', margin: '0 0 6px' }}>Comparador de fontes</h1>
          <p style={{ fontSize: 14, color: '#7A8F8E', margin: 0, lineHeight: 1.6 }}>
