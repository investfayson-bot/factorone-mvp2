...
import { getSupabaseUser, getPapelParaEmpresa } from '@/lib/supabase-route'
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const prevHistorico = Array.isArray(item.historico) ? item.historico : []
  const historico = [...prevHistorico, { em: new Date().toISOString(), quem: user.email ?? user.id, acao: 'resolvido_manual' }]
  const { error } = await service.from('work_items').update({ status: 'resolvido', resolved_at: new Date().toISOString(), historico }).eq('id', id).eq('empresa_id', empresaId)
