              ...
              <tbody>
                ...
                {fornecedoresCad.map(f => (
                  <tr key={f.id}>
                    ...
                    <td style={{ fontFamily: "'Inter', sans-serif", fontSize: 11 }}>{f.cnpj || '—'}</td>
              ...
              <tbody>
                ...
                {fornecedoresAgrupados.map(f => (
                  <tr key={f.nome}>
                    ...
                    <td style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, color: 'var(--gray-400)' }}>{f.doc || '—'}</td>
                    <td style={{ textAlign: 'center' }}>{f.qtd}</td>
                    <td style={{ textAlign: 'right', fontFamily: "'Inter', sans-serif" }}>{formatBRL(f.total)}</td>
                    <td style={{ textAlign: 'right', fontFamily: "'Inter', sans-serif", fontWeight: 700, color: f.pendente > 0 ? 'var(--red)' : 'var(--navy)' }}>{formatBRL(f.pendente)}</td>
              ...
              <tbody>
                ...
                {contasFiltradas.map(c => (
                  <tr key={c.id} style={{ opacity: c.status === 'cancelada' ? 0.5 : 1 }}>
                    ...
                    <td style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, color: isVencida(c) ? 'var(--red)' : 'inherit' }}>
                      {new Date(c.data_vencimento + 'T12:00:00').toLocaleDateString('pt-BR')}
                    </td>
                    <td style={{ textAlign: 'right', fontFamily: "'Inter', sans-serif", fontWeight: 700 }}>{formatBRL(Number(c.valor || 0))}</td>
                    <td style={{ textAlign: 'right', fontFamily: "'Inter', sans-serif", color: 'var(--teal)' }}>
              ...
              <tbody>
                ...
                {contas.filter(c => c.status === 'paga' || c.status === 'parcialmente_paga').map(c => (
                  <tr key={c.id}>
                    ...
                    <td style={{ fontFamily: "'Inter', sans-serif", fontSize: 11 }}>
                    ...
                    <td style={{ textAlign: 'right', fontFamily: "'Inter', sans-serif", fontWeight: 700, color: 'var(--teal)' }}>
            ...
            {!linkGerado ? (
              ...
              <div>
                <div style={{ padding: '16px 18px', borderRadius: 12, border: '1px solid var(--teal)', background: 'rgba(94,140,135,0.04)', marginBottom: 16 }}>
                  ...
                  <p style={{ fontSize: 12, color: 'var(--navy)', wordBreak: 'break-all', fontFamily: "'Inter', sans-serif", margin: 0 }}>{linkGerado}</p>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div className="modal-box" style={{ width: 440 }} onClick={e => e.stopPropagation()}>
            ...
            {modalPagar.chave_pix && (
              <div style={{ marginBottom: 14, padding: '8px 12px', background: '#fafafa', borderRadius: 8, border: '1px solid var(--gray-100)', fontSize: 12 }}>
                ...
                <div style={{ fontFamily: "'Inter', sans-serif", userSelect: 'all' }}>{modalPagar.chave_pix}</div>
              </div>
            )}
            <div className="modal-actions">
