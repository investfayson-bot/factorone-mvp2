  ...
  return (
    <>
      ...
      {cfg && (
        <>
          ...
          <div className="txs-card" style={{ padding: '18px 20px', marginBottom: 16 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', marginBottom: 10 }}><i className="fa-solid fa-link" style={{ color: 'var(--sage)', marginRight: 8 }} />Seu link de agendamento</div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <code style={{ flex: 1, minWidth: 220, fontSize: 14, background: 'var(--surface-2)', border: '1px solid var(--line)', borderRadius: 8, padding: '10px 12px', overflow: 'auto', whiteSpace: 'nowrap', fontFamily: 'var(--font-mono)' }}>{link}</code>
              <button className="btn-action" style={{ fontSize: 14 }} onClick={() => { void navigator.clipboard.writeText(link); toast.success('Link copiado') }}><i className="fa-solid fa-copy" style={{ marginRight: 6 }} />Copiar</button>
              <a href={link} target="_blank" rel="noreferrer" className="btn-ghost" style={{ fontSize: 14, textDecoration: 'none' }}><i className="fa-solid fa-arrow-up-right-from-square" style={{ marginRight: 6 }} />Abrir</a>
            </div>
          </div>

          {/* Disponibilidade */}
          <div className="txs-card" style={{ padding: '18px 20px', marginBottom: 16 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', marginBottom: 14 }}>Disponibilidade</div>
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {DIAS.map(([lbl, n]) => { const on = cfg.dias.includes(n); return (
                  <button key={n} onClick={() => toggleDia(n)} style={{ padding: '7px 14px', borderRadius: 100, cursor: 'pointer', fontSize: 14, fontWeight: 600, border: `1px solid ${on ? 'var(--sage)' : 'var(--line)'}`, background: on ? 'var(--sage)' : 'var(--surface)', color: on ? '#fff' : 'var(--ink-mut)' }}>{lbl}</button>
                ) })}
              </div>
            </div>
      ...
      <div className="txs-card">
        <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--line)', fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>Próximas reuniões</div>
        {reunioes.length === 0 ? (
          <div style={{ padding: 36, textAlign: 'center', color: 'var(--ink-mut)', fontSize: 15 }}>Nenhuma reunião marcada. Compartilhe seu link.</div>
        ) : reunioes.map((r, i) => (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 18px', borderBottom: i < reunioes.length - 1 ? '1px solid var(--line-soft)' : 'none' }}>
            <div style={{ width: 46, textAlign: 'center', flexShrink: 0 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--ink-mut)' }}>{r.data.slice(5, 7)}/{r.data.slice(2, 4)}</div>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14.5, fontWeight: 600, color: 'var(--ink)' }}>{r.titulo}</div>
              <div style={{ fontSize: 13, color: 'var(--ink-mut)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.hora_inicio ?? ''} · {r.descricao ?? ''}</div>
            </div>
            <span style={{ fontSize: 11.5, fontWeight: 600, padding: '3px 9px', borderRadius: 100, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--gold)', background: 'var(--gold-tint)' }}>{r.status}</span>
          </div>
        ))}
      </div>
    </>
