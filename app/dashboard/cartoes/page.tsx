    ...
    <>
        ...
        <div className="kpi" style={{ borderTop: '3px solid #13201D' }}>
          <div className="kpi-lbl">Limite total
            <div style={{ width: 28, height: 28, borderRadius: 8, background: '#F1ECE1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fa-solid fa-credit-card" style={{ fontSize: 12, color: '#13201D' }} />
            </div>
          </div>
        ...
        <div className="kpi" style={{ borderTop: `3px solid ${usoPct > 80 ? '#B0413E' : usoPct > 60 ? '#B08A3E' : '#3D7A6E'}` }}>
          <div className="kpi-lbl">Usado este mês
            <div style={{ width: 28, height: 28, borderRadius: 8, background: usoPct > 80 ? '#F4E4E1' : '#F3ECDA', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fa-solid fa-gauge-high" style={{ fontSize: 12, color: usoPct > 80 ? '#B0413E' : '#B08A3E' }} />
            </div>
          </div>
        ...
        <div className="kpi" style={{ borderTop: '3px solid #3D7A6E' }}>
          <div className="kpi-lbl">Disponível
            <div style={{ width: 28, height: 28, borderRadius: 8, background: '#E9F0ED', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fa-solid fa-circle-check" style={{ fontSize: 12, color: '#3D7A6E' }} />
            </div>
          </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 16, overflowX: 'auto', paddingBottom: 4 }}>
            ...
            >
              ...
              <span style={{ fontSize: 11, color: '#7B8C88', fontWeight: 600 }}>Novo cartão</span>
            </div>
          </div>
              ...
              <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, padding: '16px 18px' }}>
                ...
                <ResponsiveContainer width="100%" height={140}>
                  <BarChart data={historicoMeses} margin={{ top: 4, right: 4, bottom: 0, left: 0 }} barSize={28}>
                    ...
                    <YAxis tick={{ fontSize: 9, fill: '#A6B0AC' }} axisLine={false} tickLine={false} tickFormatter={v => `R$${(v/1000).toFixed(0)}K`} width={40} />
                    <Tooltip formatter={(v: number) => [formatBRL(v), 'Gastos']} contentStyle={{ fontSize: 11, borderRadius: 8, border: '0.5px solid #E4DCCC' }} />
          ...
          {tab === 'gastos' && (
            <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
              ...
              {gastosDoCarto.length === 0 ? (
                <div style={{ padding: '40px 16px', textAlign: 'center', color: '#7B8C88', fontSize: 12 }}>Nenhum gasto registrado neste cartão.</div>
                ...
                <div key={g.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr 100px 90px 90px', padding: '11px 16px', borderBottom: i < gastosDoCarto.length - 1 ? '0.5px solid #EFE9DC' : 'none', alignItems: 'center' }}>
                  <div style={{ fontSize: 11, color: '#7B8C88' }}>{new Date(g.data + 'T12:00:00').toLocaleDateString('pt-BR')}</div>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#13201D' }}>{g.descricao}</div>
                    {g.estabelecimento && <div style={{ fontSize: 10, color: '#7B8C88' }}>{g.estabelecimento}</div>}
                  </div>
                  <div>
                    <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 20, background: `${CATS_CORES[g.categoria || 'Outros'] || '#7B8C88'}18`, color: CATS_CORES[g.categoria || 'Outros'] || '#7B8C88', fontWeight: 600 }}>{g.categoria || 'Outros'}</span>
                  </div>
            ...
            return (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 12, alignItems: 'start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
                    <div style={{ padding: '12px 16px', borderBottom: '0.5px solid #E4DCCC', background: '#FBF8F1', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: '#13201D', textTransform: 'capitalize' }}>Fatura aberta{aberta ? ` · ${labelComp(aberta.comp)}` : ''}</div>
                      <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 20, background: '#F3ECDA', color: '#B08A3E' }}>ABERTA</span>
                    </div>
                    {!aberta || aberta.itens.length === 0 ? (
                      <div style={{ padding: '28px 16px', textAlign: 'center', color: '#A6B0AC', fontSize: 12 }}>Nenhum lançamento na fatura aberta.</div>
                      ...
                      <div key={g.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', borderBottom: i < aberta.itens.length - 1 ? '0.5px solid #EFE9DC' : 'none' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 30, height: 30, borderRadius: 8, background: `${CATS_CORES[g.categoria || 'Outros'] || '#7B8C88'}18`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <i className="fa-solid fa-receipt" style={{ fontSize: 12, color: CATS_CORES[g.categoria || 'Outros'] || '#7B8C88' }} />
                          </div>
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 600, color: '#13201D' }}>{g.descricao}</div>
                  ...
                  {fechadas.length > 0 && (
                    <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
                      <div style={{ padding: '12px 16px', borderBottom: '0.5px solid #E4DCCC', background: '#FBF8F1', fontSize: 12, fontWeight: 700, color: '#13201D' }}>Faturas anteriores</div>
                      {fechadas.map((f, i) => (
                        <div key={f.comp} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: i < fechadas.length - 1 ? '0.5px solid #EFE9DC' : 'none' }}>
                          ...
                          {f.status === 'paga' ? (
                            ...
                            <button onClick={() => void pagarFatura(f.comp, f.total)} style={{ fontSize: 11, fontWeight: 700, padding: '5px 12px', borderRadius: 8, border: 'none', background: '#13201D', color: '#fff', cursor: 'pointer' }}>Pagar</button>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ background: '#13201D', borderRadius: 14, padding: '18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#6FA595', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Fatura aberta</div>
                  {[
                  ...
                  <div key={c.id} style={{ background: '#fff', border: `0.5px solid ${c.id === cartaoSel ? '#3D7A6E' : '#E4DCCC'}`, borderRadius: 12, padding: '16px 18px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 13, fontWeight: 700, color: '#13201D' }}>{c.nome}</div>
                          <div style={{ fontSize: 10, color: '#7B8C88' }}>{c.bandeira} · Vence dia {c.vencimento_dia}</div>
                        </div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
          ...
          {tab === 'equipe' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 11, color: '#7B8C88', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                return (
                  <div key={c.id} style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 12, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', opacity: c.pausado ? 0.65 : 1 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 160 }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#13201D' }}>{c.titular_nome || 'Sem titular'}</div>
                      <div style={{ fontSize: 11, color: '#7B8C88' }}>{c.nome} · {c.formato === 'virtual' ? 'Virtual' : 'Físico'}{c.titular_email ? ` · ${c.titular_email}` : ''}</div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => { const v = prompt(`Novo limite para ${c.nome} (R$):`, String(c.limite)); if (v) void ajustarLimite(c, Number(v)) }} style={{ fontSize: 11, padding: '6px 12px', borderRadius: 8, border: '0.5px solid #E4DCCC', background: '#fff', color: '#3C4A46', cursor: 'pointer', fontWeight: 600 }}>
                        <i className="fa-solid fa-sliders" style={{ marginRight: 5 }} />Limite
                      </button>
                      <button onClick={() => void togglePausar(c)} style={{ fontSize: 11, padding: '6px 12px', borderRadius: 8, border: 'none', background: c.pausado ? '#E9F0ED' : '#F4E4E1', color: c.pausado ? '#2B564D' : '#B0413E', cursor: 'pointer', fontWeight: 700 }}>
        ...
        <div className="modal-bg" onClick={e => e.target === e.currentTarget && setModalCartao(false)}>
          <div className="modal-box">
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <div><h3 className="modal-title">Novo cartão</h3><div style={{ fontSize: 11, color: '#7B8C88' }}>Cadastre um cartão corporativo</div></div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Tipo do cartão</label>
                <div style={{ display: 'flex', gap: 8 }}>
                  {[{ v: 'fisico', l: 'Físico', i: 'fa-credit-card' }, { v: 'virtual', l: 'Virtual', i: 'fa-wifi' }].map(o => (
                    <button key={o.v} onClick={() => setFormCartao(p => ({ ...p, formato: o.v }))} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '9px', borderRadius: 9, border: formCartao.formato === o.v ? '1px solid #3D7A6E' : '0.5px solid #E4DCCC', background: formCartao.formato === o.v ? '#E9F0ED' : '#fff', color: formCartao.formato === o.v ? '#2B564D' : '#7B8C88', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Titular (colaborador)</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>E-mail do titular</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Bandeira</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Limite (R$) *</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Fechamento (dia)</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Vencimento (dia)</label>
              ...
              <div>
                <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Cor do cartão</label>
                <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 22, justifyContent: 'flex-end' }}>
              <button onClick={() => setModalCartao(false)} style={{ padding: '8px 16px', borderRadius: 8, border: '0.5px solid #E4DCCC', background: 'transparent', color: '#7B8C88', fontSize: 12, cursor: 'pointer' }}>Cancelar</button>
      ...
      {modalGasto && (
        <div className="modal-bg" onClick={e => e.target === e.currentTarget && setModalGasto(false)}>
          <div className="modal-box">
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <div><h3 className="modal-title">Registrar gasto</h3><div style={{ fontSize: 11, color: '#7B8C88' }}>Lançar no cartão: {cartaoAtual?.nome}</div></div>
              <button className="modal-close" onClick={() => setModalGasto(false)}><i className="fa-solid fa-xmark" /></button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Descrição *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Valor (R$) *</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Categoria</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Data</label>
                  <input className="form-input" type="date" value={formGasto.data} onChange={e => setFormGasto(p => ({ ...p, data: e.target.value }))} />
                </div>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Parcelas</label>
              ...
              <div>
                <label style={{ fontSize: 10, fontWeight: 600, color: '#7B8C88', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Estabelecimento</label>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 22, justifyContent: 'flex-end' }}>
              <button onClick={() => setModalGasto(false)} style={{ padding: '8px 16px', borderRadius: 8, border: '0.5px solid #E4DCCC', background: 'transparent', color: '#7B8C88', fontSize: 12, cursor: 'pointer' }}>Cancelar</button>
