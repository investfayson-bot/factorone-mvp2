    ...
    <>
      <div className="page-hdr">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button className="btn-action" style={{ fontSize: 14 }} onClick={() => void gerar()} disabled={gerando} title="Gera os lançamentos (partida dobrada) das despesas e notas">
            <i className="fa-solid fa-wand-magic-sparkles" style={{ marginRight: 6 }} />{gerando ? 'Gerando…' : 'Gerar lançamentos'}
          </button>
          <select className="form-input" style={{ width: 'auto', padding: '6px 10px', fontSize: 14 }} value={meses} onChange={e => setMeses(Number(e.target.value))}>
      ...
      <div style={{ display: 'flex', gap: 2, background: '#F1ECE1', padding: 3, borderRadius: 10, width: 'fit-content', marginBottom: 16, flexWrap: 'wrap' }}>
        {([
          ...
          <button key={t.k} onClick={() => { setVista(t.k); setContaSel(null) }} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13.5, fontWeight: vista === t.k ? 700 : 500, padding: '6px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', background: vista === t.k ? '#fff' : 'transparent', color: vista === t.k ? '#13201D' : '#7B8C88' }}>
      ...
      {vista === 'balancete' && (loading ? (
        <div style={{ padding: 32, color: 'var(--gray-400)', fontSize: 15 }}>Carregando livros…</div>
      ) : lancs.length === 0 ? (
        <div style={{ background: '#fff', border: '1.5px dashed #D1D9D8', borderRadius: 16, padding: '48px 24px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: '#7B8C88' }}>Os lançamentos são gerados das notas, despesas e conciliação. Categorize e concilie para o balancete se formar.</div>
        </div>
      ) : !contaSel ? (
        <>
          {/* KPIs do balancete */}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Balancete de verificação · {balancete.length} contas</div>
            <button className="btn-ghost" style={{ fontSize: 14 }} onClick={exportarBalancete}><i className="fa-solid fa-file-csv" style={{ marginRight: 6 }} />Baixar CSV</button>
          </div>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 120px 130px', padding: '10px 16px', background: '#FBF8F1', borderBottom: '0.5px solid #E4DCCC' }}>
              {['Conta', 'Débito', 'Crédito', 'Saldo'].map((h, i) => <div key={h} style={{ fontSize: 12, fontWeight: 600, color: '#7B8C88', textTransform: 'uppercase', letterSpacing: '.06em', textAlign: i === 0 ? 'left' : 'right' }}>{h}</div>)}
            </div>
            {balancete.map((r, i) => (
              <button key={r.conta} onClick={() => setContaSel(r.conta)} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 120px 130px', padding: '11px 16px', borderBottom: i < balancete.length - 1 ? '0.5px solid #EFE9DC' : 'none', width: '100%', border: 'none', background: '#fff', cursor: 'pointer', alignItems: 'center' }}>
                <div style={{ fontSize: 14.5, fontWeight: 600, color: '#13201D', textAlign: 'left' }}>{r.conta}</div>
                <div style={{ fontSize: 14, textAlign: 'right', color: '#374151', fontFamily: "var(--font-sans)" }}>{r.debito ? formatBRL(r.debito) : '—'}</div>
                <div style={{ fontSize: 14, textAlign: 'right', color: '#374151', fontFamily: "var(--font-sans)" }}>{r.credito ? formatBRL(r.credito) : '—'}</div>
                <div style={{ fontSize: 14.5, textAlign: 'right', fontWeight: 700, color: r.saldo >= 0 ? '#13201D' : '#B0413E', fontFamily: "var(--font-sans)" }}>{formatBRL(r.saldo)}</div>
              </button>
            ))}
          </div>
        </>
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <button className="btn-ghost" style={{ fontSize: 14 }} onClick={() => setContaSel(null)}><i className="fa-solid fa-arrow-left" style={{ marginRight: 6 }} />Balancete</button>
            <div style={{ fontSize: 14, fontWeight: 800, color: '#13201D' }}>Razão · {contaSel}</div>
            <span style={{ fontSize: 14, color: '#7B8C88', marginLeft: 'auto' }}>{razao.length} lançamentos</span>
          </div>
          <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 110px 110px 120px', padding: '10px 16px', background: '#FBF8F1', borderBottom: '0.5px solid #E4DCCC' }}>
              {['Compet.', 'Histórico', 'Débito', 'Crédito', 'Saldo'].map((h, i) => <div key={h} style={{ fontSize: 12, fontWeight: 600, color: '#7B8C88', textTransform: 'uppercase', letterSpacing: '.06em', textAlign: i < 2 ? 'left' : 'right' }}>{h}</div>)}
            </div>
            {razao.map((l, i) => (
              <div key={l.id} style={{ display: 'grid', gridTemplateColumns: '90px 1fr 110px 110px 120px', padding: '10px 16px', borderBottom: i < razao.length - 1 ? '0.5px solid #EFE9DC' : 'none', alignItems: 'center' }}>
                <div style={{ fontSize: 13, color: '#7B8C88', fontFamily: "var(--font-sans)" }}>{l.competencia?.slice(0, 7)}</div>
                <div style={{ fontSize: 14.5, color: '#13201D' }}>{l.descricao} <span style={{ fontSize: 12, color: '#A6B0AC' }}>· {l.origem}</span></div>
                <div style={{ fontSize: 14, textAlign: 'right', color: l.tipo === 'debito' ? '#374151' : '#C4CFCE', fontFamily: "var(--font-sans)" }}>{l.tipo === 'debito' ? formatBRL(num(l.valor)) : '—'}</div>
                <div style={{ fontSize: 14, textAlign: 'right', color: l.tipo === 'credito' ? '#374151' : '#C4CFCE', fontFamily: "var(--font-sans)" }}>{l.tipo === 'credito' ? formatBRL(num(l.valor)) : '—'}</div>
                <div style={{ fontSize: 14.5, textAlign: 'right', fontWeight: 700, color: l.saldo >= 0 ? '#13201D' : '#B0413E', fontFamily: "var(--font-sans)" }}>{formatBRL(l.saldo)}</div>
              </div>
            ))}
          </div>
        </>
      ...
      {vista === 'balanco' && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', background: '#FBF8F1', borderBottom: '0.5px solid #E4DCCC', fontSize: 14, fontWeight: 700, color: '#13201D' }}>ATIVO</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '11px 16px' }}>
                <span style={{ fontSize: 14.5, color: '#374151', fontWeight: 700 }}>Total do ativo</span>
                <span style={{ fontSize: 14.5, fontWeight: 700, color: '#13201D', fontFamily: "var(--font-sans)" }}>{formatBRL(balanco.ativo)}</span>
              </div>
            </div>
            <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', background: '#FBF8F1', borderBottom: '0.5px solid #E4DCCC', fontSize: 14, fontWeight: 700, color: '#13201D' }}>PASSIVO + PATRIMÔNIO LÍQUIDO</div>
              {[
                ...
                <div key={r.nome} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', borderBottom: i < arr.length - 1 ? '0.5px solid #EFE9DC' : 'none' }}>
                  <span style={{ fontSize: 14.5, color: '#374151', fontWeight: r.bold ? 700 : 400 }}>{r.nome}</span>
                  <span style={{ fontSize: 14.5, fontWeight: 700, color: r.valor >= 0 ? '#13201D' : '#B0413E', fontFamily: "var(--font-sans)" }}>{formatBRL(r.valor)}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{ marginTop: 12, padding: '12px 16px', borderRadius: 12, background: Math.abs(balanco.diferenca) < 0.01 ? 'rgba(61,122,110,.08)' : 'rgba(176,138,62,.08)', border: `1px solid ${Math.abs(balanco.diferenca) < 0.01 ? 'rgba(61,122,110,.25)' : 'rgba(176,138,62,.25)'}`, fontSize: 14.5, color: '#13201D', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {vista === 'fechamento' && (
        <>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 10 }}>Fechamento mensal · {competencias.length} competências</div>
          {competencias.length === 0 ? (
            <div style={{ background: '#fff', border: '1.5px dashed #D1D9D8', borderRadius: 14, padding: '40px', textAlign: 'center', color: '#7B8C88', fontSize: 14 }}>Sem competências com lançamentos ainda.</div>
          ) : (
            <div style={{ background: '#fff', border: '0.5px solid #E4DCCC', borderRadius: 14, overflow: 'hidden' }}>
              {competencias.map((c, i) => {
                ...
                return (
                  <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: i < competencias.length - 1 ? '0.5px solid #EFE9DC' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: '#13201D', textTransform: 'capitalize' }}>{label}</div>
                      <div style={{ fontSize: 12.5, color: fech ? '#7B8C88' : '#2B564D' }}>{fech ? 'Período fechado' : 'Período aberto'}</div>
                    </div>
                    <button onClick={() => void fecharComp(c, !fech)} disabled={busyComp === c} style={{ fontSize: 13.5, fontWeight: 700, padding: '6px 14px', borderRadius: 8, border: fech ? '0.5px solid #E4DCCC' : 'none', background: fech ? '#fff' : '#13201D', color: fech ? '#3C4A46' : '#fff', cursor: 'pointer' }}>
