...
export default function DespesasPage() {
  ...
  async function marcarPago(id: string) {
    ...
    const { data: tx, error: e1 } = await supabase.from('transacoes').insert({ empresa_id: userId, data: hoje, descricao: `Despesa: ${row.descricao}`, categoria: row.categoria, tipo: 'saida', valor: row.valor, status: 'confirmada' }).select('id').single()
  ...
  async function marcarPagoLote() {
    ...
    for (const id of pendentes) {
      const row = rows.find((r) => r.id === id)
      if (!row) continue
      const { data: tx } = await supabase.from('transacoes').insert({ empresa_id: userId, data: hoje, descricao: `Despesa: ${row.descricao}`, categoria: row.categoria, tipo: 'saida', valor: row.valor, status: 'confirmada' }).select('id').single()
