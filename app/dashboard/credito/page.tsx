...
const ESTRATEGIAS = [
  ...
  { titulo: 'Consórcio como alternativa', icon: 'fa-handshake', cor: '#D97706', desc: 'Sem juros (só taxa adm.). Ideal para quem não tem urgência e quer parcelar com custo baixo. Risco: sorteio/lance para contemplação.' },
  { titulo: 'Diversificar fontes de crédito', icon: 'fa-scale-balanced', cor: '#E74C3C', desc: 'Não concentre todo crédito num só banco. Fintech + banco tradicional + BNDES = melhores condições e menor dependência.' },
]
...
export default function CreditoPage() {
  ...
  return (
    <>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 2, background: '#E8EDEC', padding: 3, borderRadius: 10, width: 'fit-content', marginBottom: 20 }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setModo(t.key)} style={{
            ...
            color: modo === t.key ? '#1C2B2A' : '#7A8F8E', transition: 'all .15s',
          }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 14 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, padding: '16px 18px' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', marginBottom: 14, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Parâmetros</div>
              {[
                ...
                <div key={f.label} style={{ marginBottom: 12 }}>
                  ...
                  <input className="form-input" type="number" placeholder={f.placeholder} value={f.value} onChange={e => f.set(e.target.value)} style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 600 }} />
                </div>
              ))}
              <div>
                <label style={{ fontSize: 10, fontWeight: 600, color: '#7A8F8E', display: 'block', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Sistema de amortização</label>
                <div style={{ display: 'flex', gap: 2, background: '#E8EDEC', padding: 3, borderRadius: 8 }}>
                  {(['price', 'sac'] as const).map(s => (
                    <button key={s} onClick={() => setSistema(s)} style={{
                      ...
                      color: sistema === s ? '#1C2B2A' : '#7A8F8E',
                    }}>{s.toUpperCase()}</button>
                  ))}
                </div>
                <div style={{ fontSize: 10, color: '#7A8F8E', marginTop: 6, lineHeight: 1.5 }}>
              ...
              <div style={{ background: '#1C2B2A', borderRadius: 14, padding: '16px 18px' }}>
                ...
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '0.5px solid rgba(255,255,255,.06)' }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: item.cor, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{item.val}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {comparativo.length > 0 && (
              <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, padding: '16px 18px' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', marginBottom: 4, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Price vs SAC — parcelas (primeiros 12 meses)</div>
                <div style={{ fontSize: 11, color: '#7A8F8E', marginBottom: 14 }}>Price tem parcelas fixas. SAC começa maior mas você paga menos juros no total.</div>
            ...
            {tabela.length > 0 && (
              <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '0.5px solid #E2E8E7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#F8FAFA' }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Tabela de amortização — {sistema.toUpperCase()}</div>
                  <span style={{ fontSize: 10, color: '#7A8F8E' }}>mostrando {Math.min(24, tabela.length)} de {tabela.length} meses</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 1fr 1fr 1fr', padding: '9px 16px', background: '#F8FAFA', borderBottom: '0.5px solid #E2E8E7' }}>
                ...
                <div style={{ maxHeight: 300, overflowY: 'auto' }}>
                  {tabela.slice(0, 24).map((r, i) => (
                    <div key={r.mes} style={{ display: 'grid', gridTemplateColumns: '60px 1fr 1fr 1fr 1fr', padding: '8px 16px', borderBottom: i < Math.min(24, tabela.length) - 1 ? '0.5px solid #F0F4F3' : 'none', alignItems: 'center' }}>
                      <div style={{ fontSize: 11, color: '#7A8F8E', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{r.mes}</div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: '#1C2B2A', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{formatBRL(r.parcela)}</div>
                      <div style={{ fontSize: 11, color: '#E74C3C', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{formatBRL(r.juros)}</div>
                      <div style={{ fontSize: 11, color: '#5E8C87', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{formatBRL(r.amortizacao)}</div>
                      <div style={{ fontSize: 11, color: '#7A8F8E', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{formatBRL(r.saldo)}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {modo === 'consorcio' && (
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 14 }}>
          <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, padding: '16px 18px' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', marginBottom: 14, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Simular consórcio</div>
            {[
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, padding: '16px 18px' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', marginBottom: 12, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Pontos importantes sobre consórcio</div>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: 8, padding: '6px 0', borderBottom: i < 5 ? '0.5px solid #F0F4F3' : 'none', alignItems: 'flex-start' }}>
                  <i className={`fa-solid ${item.icon}`} style={{ fontSize: 11, color: item.cor, marginTop: 2, flexShrink: 0 }} />
                  <span style={{ fontSize: 12, color: '#1C2B2A', lineHeight: 1.5 }}>{item.texto}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {modo === 'credito_giro' && (
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 14 }}>
          <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, padding: '16px 18px' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', marginBottom: 14, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Dados da empresa</div>
            {[
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="kpis" style={{ gridTemplateColumns: 'repeat(3,1fr)' }}>
              ...
              <div className="kpi" style={{ borderTop: '3px solid #D97706' }}>
                ...
                <div className="kpi-val" style={{ color: '#D97706' }}>{formatBRL(limiteGiro * Number(taxa) / 100)}</div>
                <div className="kpi-delta warn">sobre limite de giro</div>
              </div>
            </div>

            <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, padding: '16px 18px' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', marginBottom: 14, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Linhas recomendadas para sua empresa</div>
              {TAXAS_MERCADO.filter(t => t.tipo === 'Empresa').map(linha => (
                <div key={linha.nome} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '0.5px solid #F0F4F3' }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#1C2B2A' }}>{linha.nome}</div>
                    <div style={{ fontSize: 10, color: '#7A8F8E' }}>Custo sobre {formatBRL(limiteGiro)}: {formatBRL(limiteGiro * linha.taxa / 100)}/mês</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: linha.taxa > 2 ? '#D97706' : '#5E8C87', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{linha.taxa}%/mês</div>
      ...
      {modo === 'comparativo' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {/* Gráfico comparativo */}
          <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, padding: '16px 18px' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', marginBottom: 4, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Taxas de crédito no mercado (Junho 2026)</div>
            <div style={{ fontSize: 11, color: '#7A8F8E', marginBottom: 14 }}>Taxas mensais — quanto menor, melhor. Cartão e cheque especial devem ser evitados.</div>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={TAXAS_MERCADO} layout="vertical" margin={{ top: 0, right: 60, bottom: 0, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#F0F4F3" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 9, fill: '#AAB8B7' }} axisLine={false} tickLine={false} tickFormatter={v => `${v}%`} />
                <YAxis type="category" dataKey="nome" tick={{ fontSize: 10, fill: '#1C2B2A' }} axisLine={false} tickLine={false} width={170} />
          ...
          {/* Estratégias */}
          <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Estratégias para conseguir mais crédito</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
            {ESTRATEGIAS.map(e => (
              <div key={e.titulo} style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#1C2B2A', lineHeight: 1.3 }}>{e.titulo}</div>
                </div>
                <div style={{ fontSize: 11, color: '#7A8F8E', lineHeight: 1.6 }}>{e.desc}</div>
              </div>
            ))}
          </div>
        </div>
