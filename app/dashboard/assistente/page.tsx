      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 14, alignItems: 'start' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {msgs.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, flexDirection: m.autor === 'user' ? 'row-reverse' : 'row' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: m.autor === 'ai' ? 'var(--sage)' : 'var(--ink)', color: '#fff', fontSize: 14 }}>
                ...
                <div style={{ maxWidth: '80%', display: 'flex', flexDirection: 'column', gap: 8, alignItems: m.autor === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{ padding: '10px 14px', borderRadius: 12, fontSize: 15, lineHeight: 1.6, whiteSpace: 'pre-wrap', background: m.autor === 'ai' ? 'var(--surface-2)' : 'var(--ink)', color: m.autor === 'ai' ? 'var(--ink)' : 'var(--paper)', border: m.autor === 'ai' ? '1px solid var(--line)' : 'none' }}>{m.texto}</div>
                  {m.acao && (
                    <button onClick={() => void executarAcao(m.acao!, i)} disabled={pensando || gerando}
                      style={{ alignSelf: 'flex-start', fontSize: 14, fontWeight: 700, padding: '8px 14px', borderRadius: 9, border: 'none', background: 'var(--sage)', color: '#fff', cursor: pensando || gerando ? 'default' : 'pointer', opacity: pensando || gerando ? .6 : 1, display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  {m.relatorio && (
                    <div style={{ background: 'var(--surface)', border: '1px solid var(--sage)', borderRadius: 12, padding: '12px 14px', width: 260, boxShadow: 'var(--shadow-card)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--ink)' }}>Relatório do dia</div>
                          <div style={{ fontSize: 12.5, color: 'var(--ink-mut)' }}>Resultado {formatBRL(m.relatorio.r.resultado)}</div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button className="btn-action" style={{ fontSize: 13.5, padding: '6px 10px', flex: 1 }} onClick={() => setRelAberto(m.relatorio!)}><i className="fa-solid fa-up-right-and-down-left-from-center" style={{ marginRight: 5 }} />Abrir</button>
                        <button className="btn-ghost" style={{ fontSize: 13.5, padding: '6px 10px' }} title="WhatsApp" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(montarTexto(m.relatorio!))}`, '_blank')}><i className="fa-brands fa-whatsapp" style={{ color: '#25D366' }} /></button>
                        <button className="btn-ghost" style={{ fontSize: 13.5, padding: '6px 10px' }} title="E-mail" onClick={() => window.open(`mailto:?subject=${encodeURIComponent('Relatório FactorOne')}&body=${encodeURIComponent(montarTexto(m.relatorio!))}`)}><i className="fa-solid fa-envelope" /></button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {(pensando || gerando) && <div style={{ fontSize: 14, color: 'var(--ink-mut)', paddingLeft: 40 }}><i className="fa-solid fa-circle-notch fa-spin" style={{ marginRight: 6 }} />{gerando ? 'montando seu relatório…' : 'pensando…'}</div>}
            <div ref={fim} />
          </div>
          <div style={{ borderTop: '1px solid var(--line)', padding: 12 }}>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
              <button onClick={() => void gerarRelatorio()} disabled={gerando || pensando} style={{ fontSize: 13, fontWeight: 700, padding: '5px 11px', borderRadius: 100, border: '1px solid var(--sage)', background: 'var(--sage-tint)', color: 'var(--sage-deep)', cursor: 'pointer' }}><i className="fa-solid fa-file-lines" style={{ marginRight: 5 }} />Gerar relatório</button>
              {SUGESTOES.map(s => <button key={s} onClick={() => void enviar(s)} disabled={pensando} style={{ fontSize: 13, padding: '5px 11px', borderRadius: 100, border: '1px solid var(--line)', background: 'var(--surface)', color: 'var(--ink-soft)', cursor: 'pointer' }}>{s}</button>)}
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="txs-card" style={{ padding: '16px 18px' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink-mut)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 12 }}>Resumo do dia</div>
            {!resumo ? <div style={{ fontSize: 14, color: 'var(--ink-mut)' }}>Carregando…</div> : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div><div style={{ fontSize: 13, color: 'var(--ink-mut)' }}>Resultado do período</div><div style={{ fontSize: 20, fontWeight: 700, color: resumo.resultado >= 0 ? 'var(--sage)' : '#B0413E', fontVariantNumeric: 'tabular-nums' }}>{formatBRL(resumo.resultado)}</div></div>
                <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  <div><div style={{ fontSize: 12, color: 'var(--ink-mut)' }}>Aluguel/mês</div><div style={{ fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>{formatBRL(resumo.aluguelMes)}</div></div>
                  <div><div style={{ fontSize: 12, color: 'var(--ink-mut)' }}>Imóveis</div><div style={{ fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>{resumo.imoveis}</div></div>
                  <div><div style={{ fontSize: 12, color: 'var(--ink-mut)' }}>Pipeline</div><div style={{ fontSize: 15, fontWeight: 700, color: 'var(--sage-deep)' }}>{formatBRL(resumo.pipelineAberto)}</div></div>
                </div>
                <button onClick={() => void gerarRelatorio()} disabled={gerando} style={{ marginTop: 2, fontSize: 14, fontWeight: 700, padding: '8px', borderRadius: 9, border: '1px solid var(--sage)', background: 'var(--sage-tint)', color: 'var(--sage-deep)', cursor: 'pointer' }}><i className="fa-solid fa-file-lines" style={{ marginRight: 6 }} />Gerar relatório do dia</button>
              </div>
            )}
          </div>

          <div className="txs-card" style={{ padding: '16px 18px' }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink-mut)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 12 }}>Precisa da sua atenção</div>
            {alertas.length === 0 ? (
              <div style={{ fontSize: 14.5, color: 'var(--sage-deep)' }}><i className="fa-solid fa-circle-check" style={{ marginRight: 6 }} />Tudo em dia por aqui ✓</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {alertas.map((a, i) => (
                  <Link key={i} href={a.href} style={{ textDecoration: 'none', display: 'flex', alignItems: 'flex-start', gap: 9 }}>
                    <i className={`fa-solid ${a.ic}`} style={{ fontSize: 14, color: a.cor, marginTop: 2, width: 14 }} />
                    <span style={{ fontSize: 14.5, color: 'var(--ink)', lineHeight: 1.4 }}>{a.txt}</span>
                  </Link>
                ))}
              </div>
            )}
          ...
          <div style={{ fontSize: 12.5, color: 'var(--ink-faint)', lineHeight: 1.6, padding: '0 4px' }}>
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(19,32,29,.45)', zIndex: 60, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '4vh 16px', overflowY: 'auto' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: 14, width: '100%', maxWidth: 620, boxShadow: '0 20px 60px rgba(19,32,29,.3)', overflow: 'hidden' }}>
        ...
        <div className="rel-toolbar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid var(--line)', background: 'var(--surface-2)' }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}><i className="fa-solid fa-file-lines" style={{ marginRight: 8, color: 'var(--sage-deep)' }} />Relatório do dia</div>
          <div style={{ display: 'flex', gap: 6 }}>
            <button className="btn-ghost" style={{ fontSize: 14, padding: '6px 10px' }} onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(montarTexto(rel))}`, '_blank')}><i className="fa-brands fa-whatsapp" style={{ marginRight: 5, color: '#25D366' }} />WhatsApp</button>
            <button className="btn-ghost" style={{ fontSize: 14, padding: '6px 10px' }} onClick={() => window.open(`mailto:?subject=${encodeURIComponent('Relatório FactorOne')}&body=${encodeURIComponent(montarTexto(rel))}`)}><i className="fa-solid fa-envelope" style={{ marginRight: 5 }} />E-mail</button>
            <button className="btn-ghost" style={{ fontSize: 14, padding: '6px 10px' }} onClick={copiar}><i className="fa-solid fa-copy" style={{ marginRight: 5 }} />Copiar</button>
            <button className="btn-ghost" style={{ fontSize: 14, padding: '6px 10px' }} onClick={() => window.print()}><i className="fa-solid fa-print" style={{ marginRight: 5 }} />PDF</button>
            <button className="btn-ghost" style={{ fontSize: 14, padding: '6px 9px' }} onClick={onClose}><i className="fa-solid fa-xmark" /></button>
          </div>
        </div>
        {/* Folha */}
        <div className="rel-folha" style={{ padding: '26px 30px', maxHeight: '78vh', overflowY: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--sage-deep)' }}>Factor<span style={{ color: 'var(--gold)' }}>One</span></div>
          </div>
          <div style={{ fontSize: 13.5, color: 'var(--ink-mut)', marginBottom: 20 }}>{rel.gerado}</div>

          {rel.narrativa && <div style={{ fontSize: 15, lineHeight: 1.7, color: 'var(--ink)', background: 'var(--sage-tint)', border: '1px solid var(--line)', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>{rel.narrativa}</div>}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 22 }}>
            {[
              ...
              <div key={k.l} style={{ border: '1px solid var(--line)', borderRadius: 10, padding: '12px 14px' }}>
                <div style={{ fontSize: 12.5, color: 'var(--ink-mut)', textTransform: 'uppercase', letterSpacing: '.06em' }}>{k.l}</div>
                <div style={{ fontSize: 17, fontWeight: 800, color: k.c, fontVariantNumeric: 'tabular-nums' }}>{k.v}</div>
              </div>
            ))}
          </div>

          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginBottom: 22, fontSize: 14.5 }}>
          ...
          {r.topCats?.length > 0 && (
            <div style={{ marginBottom: 22 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-mut)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Onde você gastou</div>
              {(() => { const max = Math.max(...r.topCats.map(([, v]) => v), 1); return r.topCats.slice(0, 6).map(([c, v]) => (
                <div key={c} style={{ marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14.5, marginBottom: 3 }}><span style={{ color: 'var(--ink)' }}>{c}</span><span style={{ fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{formatBRL(v)}</span></div>
          ...
          {alertas.length > 0 && (
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-mut)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Precisa de atenção</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {alertas.map((a, i) => <div key={i} style={{ fontSize: 14.5, color: 'var(--ink)', display: 'flex', gap: 8 }}><i className="fa-solid fa-circle-exclamation" style={{ color: 'var(--gold)', marginTop: 2 }} />{a}</div>)}
              </div>
            </div>
          )}
        </div>
