          ...
          <table>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={r.id}>
                  <td style={{ fontFamily: "'Manrope', system-ui, sans-serif", fontSize: 11 }}>{new Date(r.data_transacao).toLocaleString('pt-BR')}</td>
                  <td style={{ fontWeight: 600 }}>{r.descricao}</td>
                  <td style={{ color: 'var(--gray-500)' }}>{r.contraparte_nome || '—'}</td>
                  <td style={{ textAlign: 'right', fontWeight: 700, fontFamily: "'Manrope', system-ui, sans-serif", color: r.tipo === 'credito' ? 'var(--green)' : 'var(--red)' }}>
                    {r.tipo === 'credito' ? '+' : '-'}{formatBRL(Number(r.valor || 0))}
                  </td>
                  <td style={{ textAlign: 'right', fontFamily: "'Manrope', system-ui, sans-serif" }}>{formatBRL(Number(r.saldo_apos || 0))}</td>
