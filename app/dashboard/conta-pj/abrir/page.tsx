...
const ECOSYSTEM = [
  {
    ...
    bg: 'rgba(26,43,74,.04)',
    services: [
      { icon: 'fa-bolt', name: 'PIX e Pagamentos', href: '/dashboard/conta-pj/transferencias' },
      { icon: 'fa-file-lines', name: 'Boletos e Cobranças', href: '/dashboard/financeiro' },
      { icon: 'fa-credit-card', name: 'Cartões (Virtuais e Físicos)', href: '/dashboard/cartoes' },
    ],
  },
  {
    ...
    services: [
      { icon: 'fa-arrow-trend-up', name: 'Fundos de Investimento PJ', href: '/dashboard/conta-pj' },
      { icon: 'fa-landmark', name: 'Tesouro Direto PJ', href: '/dashboard/conta-pj' },
      { icon: 'fa-sack-dollar', name: 'Planejamento de Caixa', href: '/dashboard/cashflow' },
    ],
  },
  {
    ...
    services: [
      ...
      { icon: 'fa-robot', name: 'Contabilidade Automática', href: '/dashboard/contabilidade' },
    ],
  },
  {
    ...
    services: [
      { icon: 'fa-comments', name: 'Central de Ajuda', href: '/dashboard/aicfo' },
      { icon: 'fa-clock', name: 'Atendimento 24/7', href: '/dashboard/aicfo' },
      { icon: 'fa-chart-column', name: 'Módulo de Relatórios', href: '/dashboard/relatorios' },
    ],
  },
]

const MOBILE_ACTIONS = [
  { icon: 'fa-bolt', label: 'PIX' },
  { icon: 'fa-sack-dollar', label: 'Pagar' },
  { icon: 'fa-clipboard-list', label: 'Cobrar' },
  { icon: 'fa-right-left', label: 'Transferir' },
]
    ...
    <div>
        ...
        {FEATURES.map(f => (
          <div key={f.step} style={{ background: '#fff', border: '1px solid var(--gray-100)', borderRadius: 14, padding: '20px 18px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: f.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}><i className={`fa-solid ${f.icon}`} /></div>
              <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, marginBottom: 24, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: '#fff', border: '1px solid var(--gray-100)', borderRadius: 14, overflow: 'hidden' }}>
            ...
            {[
              { icon: 'fa-bolt', label: 'Pix recebido', sub: 'Cliente A', val: '+R$5.300,00', color: 'var(--green)', hora: '10:00' },
              { icon: 'fa-sack-dollar', label: 'Pagamento', sub: 'Fornecedor D', val: '-R$1.200,00', color: 'var(--red)', hora: '10:30' },
              { icon: 'fa-right-left', label: 'Transferência enviada', sub: 'Conta Poupança', val: '-R$2.000,00', color: 'var(--red)', hora: '10:46' },
              { icon: 'fa-clipboard-list', label: 'Cobrança recebida', sub: 'Fulano 12365', val: '+R$9.426,00', color: 'var(--green)', hora: '11:22' },
            ].map((tx, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 20px', borderBottom: '1px solid var(--gray-50)' }}>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: tx.color === 'var(--green)' ? 'rgba(45,155,111,.1)' : 'rgba(220,53,69,.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 }}><i className={`fa-solid ${tx.icon}`} /></div>
                <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
                  {MOBILE_ACTIONS.map(a => (
                    <div key={a.label} style={{ background: 'rgba(255,255,255,.08)', borderRadius: 4, padding: '4px', textAlign: 'center' }}>
                      <div style={{ fontSize: 10 }}><i className={`fa-solid ${a.icon}`} /></div>
                ...
                {cat.services.map(s => (
                  <Link key={s.name} href={s.href} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', borderRadius: 8, background: cat.bg, marginBottom: 4, textDecoration: 'none', cursor: 'pointer' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 14 }}><i className={`fa-solid ${s.icon}`} /></span>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--gray-100)', borderRadius: 14, padding: '24px 28px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, flexWrap: 'wrap' }}>
          {[
            { icon: 'fa-mobile-screen', label: 'Menu Principal' },
            { icon: 'fa-building-columns', label: 'Banco PJ' },
            { icon: 'fa-clipboard-list', label: 'Abertura de Conta' },
            { icon: 'fa-circle-check', label: 'Conta Ativa' },
          ].map((step, i) => (
            <div key={step.label} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '14px 20px', background: 'var(--gray-50)', borderRadius: 12, minWidth: 110, border: '1px solid var(--gray-100)' }}>
                <span style={{ fontSize: 24, marginBottom: 6 }}><i className={`fa-solid ${step.icon}`} /></span>
        ...
        <div style={{ marginTop: 24, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {[
            { icon: 'fa-bolt', title: 'Rápido', desc: 'Abertura em menos de 5 minutos' },
            { icon: 'fa-lock', title: 'Seguro', desc: 'Criptografia de ponta a ponta' },
            { icon: 'fa-money-bill-wave', title: 'Gratuito', desc: 'Sem mensalidades ou tarifas' },
            { icon: 'fa-mobile-screen', title: 'Digital', title2: '', desc: 'Gerencie tudo pelo browser' },
          ].map(b => (
            <div key={b.title} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '12px 14px', background: 'var(--gray-50)', borderRadius: 10 }}>
              <span style={{ fontSize: 22, flexShrink: 0 }}><i className={`fa-solid ${b.icon}`} /></span>
              <div>
