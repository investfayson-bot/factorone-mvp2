  ...
  return (
    <>
      ...
      <div className="kpis" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 16 }}>
        ...
        <div className="kpi" style={{ borderTop: '3px solid #2D9B6F' }}>
          <div className="kpi-lbl">Rendimento</div>
          <div className="kpi-val" style={{ color: rendimento >= 0 ? '#2D9B6F' : '#E74C3C' }}>{formatBRL(rendimento)}</div>
      ...
      <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, overflow: 'hidden', marginBottom: 16 }}>
        <div style={{ padding: '12px 16px', borderBottom: '0.5px solid #E2E8E7', background: '#F8FAFA', fontSize: 12, fontWeight: 700, color: '#1C2B2A' }}>Carteira ativa</div>
        {ativos.length === 0 ? (
          ...
          return (
            <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: i < ativos.length - 1 ? '0.5px solid #F0F4F3' : 'none' }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: '#EAF5F3', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <i className="fa-solid fa-seedling" style={{ fontSize: 14, color: '#2D9B6F' }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#1C2B2A' }}>{r.nome}</div>
                <div style={{ fontSize: 10.5, color: '#7A8F8E', textTransform: 'uppercase' }}>{r.tipo}{r.data_vencimento ? ` · venc. ${r.data_vencimento}` : ''}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#1C2B2A', fontFamily: "'Inter', system-ui, sans-serif" }}>{formatBRL(Number(r.valor_atual || 0))}</div>
                <div style={{ fontSize: 10.5, fontWeight: 600, color: rend >= 0 ? '#2D9B6F' : '#E74C3C' }}>{rend >= 0 ? '+' : ''}{formatBRL(rend)}</div>
              </div>
              <button onClick={() => void resgatar(r.id)} style={{ fontSize: 11, color: '#3A5150', background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 7, padding: '5px 11px', cursor: 'pointer', fontWeight: 600 }}>Resgatar</button>
            </div>
          )
        })}
      </div>
      ...
      {resgatados.length > 0 && (
        <div style={{ background: '#fff', border: '0.5px solid #E2E8E7', borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '0.5px solid #E2E8E7', background: '#F8FAFA', fontSize: 12, fontWeight: 700, color: '#1C2B2A' }}>Histórico de resgates</div>
          {resgatados.map((r, i) => (
            <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', borderBottom: i < resgatados.length - 1 ? '0.5px solid #F0F4F3' : 'none', fontSize: 12 }}>
              <span style={{ color: '#3A5150' }}>{r.nome} · <span style={{ textTransform: 'uppercase', color: '#7A8F8E' }}>{r.tipo}</span></span>
              <span style={{ fontWeight: 700, color: '#1C2B2A' }}>{formatBRL(Number(r.valor_atual || 0))}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="modal-bg" onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div className="modal-box">
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 18 }}>
              <div><h3 className="modal-title">Novo investimento</h3><div style={{ fontSize: 11, color: '#7A8F8E' }}>Adicione uma aplicação à carteira</div></div>
              <button className="modal-close" onClick={() => setModal(false)}><i className="fa-solid fa-xmark" /></button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ fontSize: 10, fontWeight: 600, color: '#7A8F8E', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '.07em' }}>Nome *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7A8F8E', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '.07em' }}>Tipo</label>
                  <select className="form-input" value={form.tipo} onChange={e => setForm(p => ({ ...p, tipo: e.target.value }))}>
                    {TIPOS.map(t => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7A8F8E', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '.07em' }}>Valor aplicado (R$) *</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7A8F8E', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '.07em' }}>Valor atual (R$)</label>
                ...
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: '#7A8F8E', display: 'block', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '.07em' }}>Vencimento</label>
