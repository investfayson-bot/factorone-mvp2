              ...
              return (
                <div key={c.id} style={{ marginBottom: 12, paddingBottom: 12, borderBottom: '1px solid var(--gray-100)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 11, fontFamily: "'Inter', system-ui, sans-serif", color: 'var(--gray-500)' }}>{formatBRL(Number(c.gasto))} / {c.orcamento ? formatBRL(Number(c.orcamento)) : '∞'}</span>
                  </div>
                ...
                {campanhas.map(c => {
                  ...
                  return (
                    <tr key={c.id}>
                      ...
                      <td style={{ textAlign: 'right', fontFamily: "'Inter', system-ui, sans-serif", fontSize: 12 }}>{c.orcamento ? formatBRL(Number(c.orcamento)) : '—'}</td>
                      <td style={{ textAlign: 'right', fontFamily: "'Inter', system-ui, sans-serif", fontSize: 12, color: 'var(--red)' }}>{formatBRL(Number(c.gasto))}</td>
                      <td style={{ textAlign: 'right', fontFamily: "'Inter', system-ui, sans-serif", fontSize: 12, color: 'var(--green)' }}>{formatBRL(Number(c.receita_gerada))}</td>
                ...
                <tbody>
                  ...
                  {leads.slice(0, 30).map(l => (
                    <tr key={l.id}>
                      ...
                      <td style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 11 }}>{new Date(l.created_at).toLocaleDateString('pt-BR')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              return (
                <div key={c.id} style={{ background: '#fff', border: '1px solid var(--gray-100)', borderRadius: 10, padding: '12px 14px', borderLeft: `4px solid ${st.color}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 10, color: 'var(--gray-400)', fontFamily: "'Inter', system-ui, sans-serif" }}>{c.data_pub ? new Date(c.data_pub + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' }) : '—'}</span>
                  </div>
