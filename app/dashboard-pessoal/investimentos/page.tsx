    ...
    <>
      ...
      {porTipo.length === 0 ? (
        <div style={{ background: '#fff', border: '2px dashed var(--gray-100)', borderRadius: 16, padding: 48, textAlign: 'center' }}>
          <i className="fa-solid fa-chart-line" style={{ fontSize: 32, color: "var(--gray-300)", marginBottom: 12, display: "block" }} />
                ...
                {grupo.map(inv => {
                  ...
                  return (
                    <tr key={inv.id}>
                      ...
                      <td style={{ fontFamily: "'Manrope', 'Inter', sans-serif" }}>{formatBRL(Number(inv.valor_aplicado))}</td>
                      <td style={{ fontFamily: "'Manrope', 'Inter', sans-serif", fontWeight: 700 }}>{formatBRL(Number(inv.valor_atual))}</td>
                      <td style={{ fontFamily: "'Manrope', 'Inter', sans-serif", color: ganho >= 0 ? 'var(--teal)' : 'var(--red)', fontSize: 11 }}>
