            ...
            <tbody>
              {ativas.map(a => (
                <tr key={a.id}>
                  ...
                  <td style={{ fontFamily: "'Inter', system-ui, sans-serif", fontWeight: 700, color: 'var(--red)' }}>{formatBRL(Number(a.valor))}</td>
            ...
            <tbody>
              {inativas.map(a => (
                <tr key={a.id} style={{ opacity: 0.6 }}>
                  <td style={{ fontWeight: 600 }}>{a.nome}</td>
                  <td><span className="tag gray">{a.categoria}</span></td>
                  <td style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>{formatBRL(Number(a.valor))}</td>
                  <td style={{ display: 'flex', gap: 6 }}>
