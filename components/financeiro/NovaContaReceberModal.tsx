    ...
    >
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 7 }}>
            {CATS_RECEBER.map((c) => (
              <button key={c.label} type="button" onClick={() => setCategoria(c.label)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, padding: '9px 6px', borderRadius: 9, cursor: 'pointer', fontSize: 10, fontWeight: 600, border: categoria === c.label ? '2px solid var(--teal)' : '1px solid var(--gray-100)', background: categoria === c.label ? 'rgba(16,185,129,0.08)' : '#fafafa', color: categoria === c.label ? 'var(--teal)' : 'var(--gray-500)', transition: 'all 0.15s' }}>
