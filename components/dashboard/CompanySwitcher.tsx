    ...
    <div ref={ref} style={{ position: 'relative' }}>
      <div className="sb-co" onClick={() => void abrir()} title="Trocar de empresa">
        ...
        <div>
          ...
          <div className="sb-co-plan">Trocar empresa <i className="fa-solid fa-chevron-up" style={{ fontSize: 11, marginLeft: 3 }} /></div>
        </div>
      </div>
      ...
      {aberto && (
        <div style={{ position: 'absolute', bottom: '100%', left: 0, marginBottom: 8, width: 260, background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, boxShadow: 'var(--shadow-pop)', overflow: 'hidden', zIndex: 200 }}>
          <div style={{ padding: '8px 12px', fontSize: 12, fontWeight: 700, color: 'var(--ink-mut)', textTransform: 'uppercase', letterSpacing: '.08em', borderBottom: '1px solid var(--line-soft)' }}>Suas empresas</div>
          <div style={{ maxHeight: 220, overflowY: 'auto' }}>
            {loading ? (
              <div style={{ padding: 16, fontSize: 14, color: 'var(--ink-mut)', textAlign: 'center' }}>Carregando…</div>
            ) : empresas.length === 0 ? (
              <div style={{ padding: 16, fontSize: 14, color: 'var(--ink-mut)', textAlign: 'center' }}>Nenhuma empresa encontrada.</div>
            ) : empresas.map(e => (
              <div key={e.id} onClick={() => !e.ativa && void trocar(e.id)} style={{
                ...
                <span style={{ fontSize: 14.5, fontWeight: 600, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{e.nome || 'Sem nome'}</span>
                {e.ativa && <i className="fa-solid fa-circle-check" style={{ color: 'var(--sage-deep)', fontSize: 13 }} />}
              </div>
            ))}
          </div>
          <div style={{ borderTop: '1px solid var(--line-soft)' }}>
            {mostrarNova ? (
              <div style={{ padding: 10, display: 'flex', gap: 6 }}>
                <input className="form-input" style={{ fontSize: 13, padding: '6px 8px' }} placeholder="Nome da empresa" value={novaNome} onChange={e => setNovaNome(e.target.value)} autoFocus onKeyDown={e => { if (e.key === 'Enter') void criarEmpresa() }} />
                <button className="btn-action" style={{ fontSize: 13, padding: '6px 10px', whiteSpace: 'nowrap' }} disabled={trocando} onClick={() => void criarEmpresa()}>Criar</button>
              </div>
            ) : (
              <button onClick={() => setMostrarNova(true)} style={{ width: '100%', textAlign: 'left', padding: '9px 12px', fontSize: 14, color: 'var(--sage-deep)', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>
                <i className="fa-solid fa-plus" style={{ marginRight: 6 }} />Nova empresa
              </button>
            )}
            <button onClick={onSair} style={{ width: '100%', textAlign: 'left', padding: '9px 12px', fontSize: 14, color: 'var(--ink-mut)', background: 'none', border: 'none', borderTop: '1px solid var(--line-soft)', cursor: 'pointer' }}>
