            ...
            <thead>
              <tr style={{ background: 'var(--gray-50, #f8f9fa)' }}>
                {['Número', 'Tipo', 'Destinatário', 'Valor', 'Status', 'Data', 'Ações'].map((h) => (
                  <th key={h} style={{ textAlign: 'left', padding: '10px 12px', fontSize: 10, fontWeight: 700, color: 'var(--gray-400)', textTransform: 'uppercase', fontFamily: "'Inter', system-ui, sans-serif", borderBottom: '1px solid var(--gray-100)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={r.id} style={{ borderTop: '1px solid var(--gray-100)' }}>
                  <td style={{ padding: '10px 12px', color: 'var(--navy)', fontFamily: "'Inter', system-ui, sans-serif" }}>{r.numero || '—'}</td>
                  ...
                  <td style={{ padding: '10px 12px', color: 'var(--navy)', fontFamily: "'Inter', system-ui, sans-serif", fontWeight: 600 }}>
