...
1. ~~**Banco**: desenho acima aguardando aprovação final~~ **Resolvido (2026-07-08):** desenho consolidado aprovado pelo Fayson — spec fechada em `2026-07-08-banco-module-design.md` (inclui decisão extra: matching com contas a pagar/receber entra na fila única, abordagem A com endpoint atômico `/api/banco/confirmar`).
...
3. ~~**Contador**: modelo de permissão pra contador convidado...~~ **Resolvido em parte (2026-07-08):** achado que `usuario_empresas` (migração `20260709000000_multi_empresa.sql`) já é exatamente essa base — um login acessa N empresas com um `papel` por empresa, troca de contexto via `CompanySwitcher.tsx` + `/api/empresas/trocar` (valida membership no servidor, escreve só via service role), e **nenhuma outra RLS do sistema precisou mudar** porque tudo já confia em `usuarios.empresa_id`. Um contador convidado é só mais um login com `papel = 'contador'` em `usuario_empresas` por cliente. Falta: (a) o convite por e-mail apontando pra esse fluxo — o padrão já existe em `app/dashboard/equipe/page.tsx`, hoje só entre membros da mesma empresa, precisa virar convite cross-empresa; (b) as telas específicas do lado contador (bookkeeping, documentos, pedidos, regime fiscal). Fluxo documentado em artifact `fluxo-multi-empresa-v1` (ver histórico de chat de 2026-07-08).
