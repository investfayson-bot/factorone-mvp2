...
export async function matchExato(extratoItem: ExtratoBancario, empresa_id: string): Promise<MatchResult | null> {
  ...
  if (extratoItem.tipo === 'debito') {
    const { data: pagar } = await getSupabase()
      .from('contas_pagar')
      .select('id')
    ...
    const { data: receber } = await getSupabase()
      .from('contas_receber')
      .select('id')
      .eq('empresa_id', empresa_id)
      .in('status', ['pendente', 'vencida'])
...
export async function matchFuzzy(extratoItem: ExtratoBancario, empresa_id: string): Promise<MatchResult | null> {
  ...
  if (extratoItem.tipo === 'debito') {
    const { data } = await getSupabase().from('contas_pagar').select('id,data_vencimento,valor').eq('empresa_id', empresa_id).in('status', ['pendente', 'vencida']).gte('valor', valorMin).lte('valor', valorMax).gte('data_vencimento', dataMin).lte('data_vencimento', dataMax).limit(1).maybeSingle()
    if (data) {
    ...
    const { data } = await getSupabase().from('contas_receber').select('id,data_vencimento,valor').eq('empresa_id', empresa_id).in('status', ['pendente', 'vencida']).gte('valor', valorMin).lte('valor', valorMax).gte('data_vencimento', dataMin).lte('data_vencimento', dataMax).limit(1).maybeSingle()
    if (data) {
      const diffValor = Math.abs(Number(data.valor) - valorRef) / Math.max(valorRef, 1)
      const conf = Math.max(0.7, 0.99 - diffValor * 10)
...
export async function matchDescricao(extratoItem: ExtratoBancario, empresa_id: string): Promise<MatchResult | null> {
  ...
  if (extratoItem.tipo === 'debito') {
    const { data } = await getSupabase().from('contas_pagar').select('id,fornecedor_nome').eq('empresa_id', empresa_id).in('status', ['pendente', 'vencida']).limit(50)
    ...
    const { data } = await getSupabase().from('contas_receber').select('id,cliente_nome').eq('empresa_id', empresa_id).in('status', ['pendente', 'vencida']).limit(50)
...
export async function aplicarConciliacao(extrato_id: string, referencia_id: string, tipo: string, empresa_id: string): Promise<void> {
  const { data: extrato } = await getSupabase().from('extrato_bancario').select('id,conta_id,data_transacao,valor,tipo').eq('id', extrato_id).maybeSingle()
  if (!extrato) return
  await getSupabase().from('conciliacoes').insert({ empresa_id, conta_id: extrato.conta_id, extrato_id, tipo_match: tipo, referencia_id, confidence: 1, metodo: 'exato' })
  await getSupabase().from('extrato_bancario').update({ conciliado: true }).eq('id', extrato_id)
  const dt = day(extrato.data_transacao)
  const v = Math.abs(Number(extrato.valor))
  if (tipo === 'conta_pagar') {
    await getSupabase().from('contas_pagar').update({ status: 'paga', data_pagamento: dt, valor_pago: v, extrato_id }).eq('id', referencia_id)
  }
  if (tipo === 'conta_receber') {
    await getSupabase().from('contas_receber').update({ status: 'recebida', data_recebimento: dt, valor_recebido: v, extrato_id }).eq('id', referencia_id)
  }
}
...
export async function conciliarExtrato(empresa_id: string, conta_id: string): Promise<{ conciliados: number; pendentes: number; matches: MatchResult[] }> {
  const { data: extratos } = await getSupabase().from('extrato_bancario').select('id,valor,tipo,data_transacao,descricao,conciliado').eq('empresa_id', empresa_id).eq('conta_id', conta_id).eq('conciliado', false).order('data_transacao', { ascending: false })
  let conciliados = 0
